...
REM
REM What this does:
REM   1. Stops any process holding port 80 or 5001 (old node, old caddy).
REM   2. Stops any running Caddy via its admin API.
REM   3. Starts Caddy from C:\Caddy (binds 80 + 443, reverse-proxies localhost:5001).
REM   4. Starts the node server on PORT=5001, detached (window hidden, survives this shell closing).
...
echo [run] Stopping anything on ports 80 / 5001...
powershell -NoProfile -Command "Get-NetTCPConnection -State Listen -LocalPort 80,5001 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { try { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue } catch {} }"

echo [run] Stopping any running Caddy...
"C:\Caddy\caddy.exe" stop >nul 2>&1

echo [run] Starting Caddy (80 + 443, reverse proxy to localhost:5001)...
pushd "C:\Caddy"
...
echo [run] Starting node server on PORT=5001 (hidden window, detached)...
...
echo [run] Waiting for node to bind 5001...
powershell -NoProfile -Command "$ok=$false; for ($i=0; $i -lt 30; $i++) { if (Get-NetTCPConnection -State Listen -LocalPort 5001 -ErrorAction SilentlyContinue) { $ok=$true; break }; Start-Sleep 1 }; if (-not $ok) { Write-Host '[run] WARNING: node not listening on 5001 after 30s'; exit 1 }"
