...
The UI talks to the **API during development** via the Vite dev proxy (`/api` → `http://localhost:3001`). Start the server separately or use the root script `npm run dev` from the monorepo.
...
1. Start the API (e.g. `npm run dev -w server` on port `3001`).
2. Start the client (`npm run dev -w client`).
3. Open the URL printed by Vite (e.g. 5173).

`vite.config.ts` proxies `/api` to `http://localhost:3001`, so the browser uses relative URLs like `/api/health` with no CORS config needed in dev.
...
The server listens on `PORT` (default `3001`) and serves the SPA for non-API routes.
