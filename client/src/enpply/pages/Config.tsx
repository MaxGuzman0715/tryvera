...
export default function Config() {
  ...
  useEffect(() => {
    Promise.all([api.getSettings(), api.getPromptsFull(), api.getPromptDefaults()])
      .then(([s, full, d]) => {
        setSettings({
          ...s,
    ...
    <>
      ...
      <div className="card">
        ...
        <div className="config-subsection">
          ...
          <label>Default theme by profile (JSON object as lines: profile_id = theme)</label>
          <KeyboardTextarea
            rows={4}
            value={JSON.stringify(settings.default_theme_by_profile, null, 2)}
            onValueChange={(v) => {
              try {
                const o = JSON.parse(v) as Record<string, string>;
                setSettings({ ...settings, default_theme_by_profile: o });
              } catch {
                /* ignore while typing */
              }
            }}
