...
import { api } from "../api";
import type { FollowupQa, MatchScoreSummary, ResultJson } from "../types";
...
export default function Result() {
  ...
  // Match score: model is the user's "matchScore" tier (server-resolved).
  ...
  return (
    <>
      ...
      {data.tailoring && (
        <div className="card">
          <h2>Résumé tailoring</h2>
          <p className="sub" style={{ marginTop: 0 }}>
            Mode:{" "}
            <strong>{data.tailoring.mode === "bullets" ? "bullets-based (pre-built groups)" : "untailored (original profile bullets)"}</strong>
            {data.tailoring.mode === "untailored" && " — no bullet store matched; experience kept as-is."}
          </p>

          {data.tailoring.gaps.length > 0 && (
            <p className="sub" style={{ fontSize: "0.85rem" }}>
              <strong>Gaps (K1 stacks with no pre-built bullets):</strong>{" "}
              {data.tailoring.gaps.map((g) => `${g.stack} (${g.domain})`).join(", ")}
            </p>
          )}

          {data.tailoring.companies.map((co) => (
            <div key={co.company} style={{ marginTop: "0.85rem" }}>
              <strong>{co.company}</strong>
              {co.project && <span className="sub"> — {co.project}</span>}
              <table style={{ width: "100%", fontSize: "0.8rem", borderCollapse: "collapse", margin: "0.35rem 0" }}>
                <thead>
                ...
                <tbody>
                  {co.stacks.map((s) => (
                    <tr key={s.stack}>
                      <td className="mono">
                        {s.stack}
                        {s.resolved_key !== s.stack ? <span className="sub"> →{s.resolved_key}</span> : ""}
                      </td>
                      <td>{s.domain}</td>
                      <td>{s.tier}</td>
                      <td>{s.weight}%</td>
              ...
              {(co.k2_placed.length > 0 || co.k2_skipped.length > 0) && (
                <p className="sub" style={{ fontSize: "0.8rem", margin: 0 }}>
                  <strong>K2 woven in:</strong> {co.k2_placed.join(", ") || "—"}
                  {" · "}
                  <strong>K2 dropped:</strong> {co.k2_skipped.join(", ") || "—"}
                </p>
              )}
            </div>
          ))}
          ...
          {data.tailoring.summary && (
            <>
              <h3 style={{ marginBottom: "0.25rem" }}>Generated summary</h3>
              <p style={{ marginTop: 0 }}>{data.tailoring.summary}</p>
            </>
          )}
          {data.tailoring.skills.length > 0 && (
            <>
              <h3 style={{ marginBottom: "0.25rem" }}>Generated skills</h3>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
                {data.tailoring.skills.map((s, i) => (
                  <span
                    key={i}
                    style={{ fontSize: "0.78rem", padding: "0.1rem 0.45rem", borderRadius: "999px", border: "1px solid var(--border, rgba(127,127,127,0.3))" }}
                  >
      ...
      <div className="card">
        ...
        <p className="hint" style={{ marginTop: 0 }}>
          Uses your <strong>Match score</strong> model tier (set in My settings). Scores experience level,
          relevant experience, core skills, and industry experience based on the stored profile and JD.
        </p>
