export type Experience = {
  ...
  rawLines?: string;
  notes?: string;
  /** True when this employer is a consulting / client-services firm. */
  consulting?: boolean;
};
...
export type ResultJson = {
  ...
  /** The final generated résumé as Markdown — the exact tailored output. */
...
export type TailoringDetail = {
  ...
  domain_scores: { domain: string; score: 1 | 2 | 3 }[];
  ...
  companies: {
    company: string;
    /** anchor = tailored live from variation A; consulting = variation B + industry engagements; older = stored bullets. */
...
/** How obviously the résumé reads as intentionally force-tailored to the JD. */
export type ForceTailoringReport = {
  /** 0-100; higher = looks MORE force-tailored to the JD (a red flag), lower = reads naturally. */
  score: number;
  assessment: string;
  signals: string[];
};
...
export type AppSettings = {
  ...
  /** Optional admin overrides pinning a specific model to the extraction / generation step (unset = use the step's tier). */
  llm_extraction?: LlmModelConfig | null;
  llm_generation?: LlmModelConfig | null;
};
