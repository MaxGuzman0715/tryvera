...
/**
 * Resume-generation LLM step: HEAVILY REWRITE each company's selected bullets into
 * strong senior lines aligned to key_requirements + K2, then derive BOTH the summary
 * and the skills list from those rewritten lines (not the JD). `promptTemplate` is
 * the resume prompt (from the prompt store). The pre-selected bullets are the
 * truthfulness boundary — the rewrite may rephrase/reorder/expand them but must not
 * invent facts. K2 terms with no honest home are discarded (reported in `skipped`),
 * so they never reach the bullets and therefore never reach the skills list either.
 */
export async function generateResumeContent(params: {
  ...
  companyName: string;
  roleName: string;
  /** The role's key requirements from extraction — a primary driver of the rewrite. */
  ...
  // Heavy rewrite: the model rewrites each company's bullets, so the count/order
  // may legitimately change. Accept whatever non-empty set it returns; fall back
  // to the pre-selected bullets only when a company is missing from the reply or
  // came back empty (a failed/degenerate rewrite must never blank a company).
  ...
  const companies = params.companies.map((c) => {
    ...
    const rewritten = Array.isArray(out?.bullets)
      ? out!.bullets.map((b) => String(b).trim()).filter(Boolean)
      : [];
    const bullets = rewritten.length ? rewritten : c.bullets;
    return {
      company: c.company,
      bullets,
