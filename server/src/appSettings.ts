...
export async function readSettings(): Promise<AppSettings> {
  try {
    ...
    const llm_heavy = sanitizeModel(parsed.llm_heavy, defaults.llm_heavy);
    return {
      ...defaults,
      ...parsed,
      ui_theme,
      ...
      llm_heavy,
      // Optional per-step overrides: keep only when present + valid, else leave unset.
      ...(parsed.llm_extraction ? { llm_extraction: sanitizeModel(parsed.llm_extraction, llm_heavy) } : { llm_extraction: undefined }),
      ...(parsed.llm_generation ? { llm_generation: sanitizeModel(parsed.llm_generation, llm_heavy) } : { llm_generation: undefined }),
    };
  } catch {
    return { ...defaults };
  }
