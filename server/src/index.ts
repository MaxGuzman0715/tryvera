...
import { logLlmConfig, getLlmClientForConfig, getLlmModelForConfig } from "./llmClient.js";
...
app.put("/api/config/settings", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    await writeSettings({
      ...
      llm_light: { provider: parsed.llm_light.provider, model: parsed.llm_light.model.trim() },
      llm_heavy: { provider: parsed.llm_heavy.provider, model: parsed.llm_heavy.model.trim() },
    });
...
app.post<{ id: string }>("/api/applications/:id/match-score", requireAuth, async (req, res) => {
  try {
    const loaded = await loadProfileAndResultForApp(req.params.id, req.user!);
    if (!loaded.ok) return res.status(loaded.status).json({ error: loaded.error });
    // Match score uses the user's "matchScore" function tier.
    const llm = resolveStepModel("matchScore", await readSettings(), req.user!.preferences?.llm_tiers);
