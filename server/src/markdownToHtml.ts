...
export function markdownToHtmlFragment(markdown: string, options?: MarkdownToHtmlOptions): string {
  ...
  // Education is wrapped in a keep-together div so the whole section stays on one page.
  let keepSectionOpen = false;
  const closeKeepSection = () => {
    if (!keepSectionOpen) return;
    out.push("</div>");
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith("## ")) {
      ...
      // Education stays whole on one page — open a keep-together wrapper around the section.
      if (/^education\b/i.test(title)) {
        out.push(`<div class="keep-together">`);
