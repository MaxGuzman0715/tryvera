...
export type SelectedStack = {
  ...
  bullets: string[];
  /** Added by the tier-mix rule (not a K1 stack) to guarantee ≥1 tier-1 and ≥1 tier-2. */
  addedForTierMix?: boolean;
};
...
export type CompanySelection = {
  ...
  /** Concatenated bullets in domain-weight order, before K2 injection (includes any top-up lines). */
  preInjectionBullets: string[];
  /** Extra lines generated by the tenure top-up rule (subset of preInjectionBullets). */
  topUpBullets?: string[];
...
/** Parse a résumé date like "Oct 2019" / "May 2014" / "Present" to epoch ms. */
function parseMonthYear(s: string): number | null {
  if (!s) return null;
  const t = s.trim();
  if (/present|current|now/i.test(t)) return Date.now();
  ...
  if (m) {
    ...
    if (mo != null && yr) return new Date(yr, mo, 1).getTime();
  }
  const yOnly = t.match(/\b(\d{4})\b/);
  if (yOnly) return new Date(parseInt(yOnly[1], 10), 0, 1).getTime();
  return null;
}

/** Tenure in years from start/end strings; 0 when unparseable. */
...
/** Minimum résumé line count expected for a role of the given tenure (design §7). */
function minLinesForTenure(years: number): number {
...
/**
 * Pick the best unused stack of a given tier from a company, to satisfy the
 * "≥1 tier-1 AND ≥1 tier-2 per company" rule. Highest-weighted domain wins;
 * "Shared" is never used to satisfy the mix.
 */
function pickStackForTier(
  company: CompanyBullets,
  excludeKeys: Set<string>,
  tier: 1 | 2,
  domainWeight: (domain: string) => number
): string | null {
  let best: string | null = null;
  let bestW = -Infinity;
  for (const [key, g] of Object.entries(company.stacks)) {
    if (g.tier !== tier || excludeKeys.has(key) || g.domain === "Shared") continue;
    const w = domainWeight(g.domain);
    if (w > bestW) {
      bestW = w;
      best = key;
    }
...
export async function runBulletsExperiment(params: {
  ...
  // ── Phase 3c-bis: guarantee ≥1 tier-1 AND ≥1 tier-2 per tailored company ──
  // Only applies to companies that already received at least one K1 stack; a
  // company with nothing falls back to its original bullets and is left alone.
  for (const co of orderedCompanies) {
    const arr = assignments.get(co.storeKey);
    if (!arr || arr.length === 0) continue;
    const company = store.companies[co.storeKey];
    const have = new Set(arr.map((s) => s.resolvedKey));
    const tiers = new Set(arr.map((s) => s.tier));
    const domainWeight = (d: string) => extraction.domain_breakdown[d] ?? 0;
    for (const tier of [1, 2] as const) {
      if (tiers.has(tier)) continue;
      const key = pickStackForTier(company, have, tier, domainWeight);
      if (!key) continue;
      const group = company.stacks[key];
      const weight = domainWeight(group.domain);
      const linesRequested = lineCountForWeight(weight, group.tier);
      arr.push({
        stack: key,
        resolvedKey: key,
  ...
  // The single per-company injection call ALSO does the tenure top-up (when a
  // long role's sliced bullets fall short) — no separate LLM round-trip.
  ...
  // For the tenure top-up rule: map store keys back to profile experiences and
  // pick the JD's single highest-priority requirement to anchor extra lines.
  const profileNameByStoreKey = new Map(orderedCompanies.map((o) => [o.storeKey, o.profileName]));
  const expByName = new Map(profile.experience.map((e) => [e.company, e]));
  const topRequirement =
    extraction.K1.soft?.[0]?.trim() ||
    `${extraction.primary_domain || "software"} engineering`;

  /** Build the prompt's TOP-UP block; empty instruction when none needed. */
  function topUpBlock(storeKey: string, need: number): string {
    if (need <= 0) return "No top-up requested. Do NOT add any new bullets.";
    const projectClause = store.companies[storeKey].project
      ? ` (project: ${store.companies[storeKey].project})`
      : "";
    return [
      `Append exactly ${need} NEW bullet(s) AFTER the input bullets for ${storeKey}${projectClause}.`,
      `They must directly demonstrate this top job requirement: "${topRequirement}".`,
      `Make them consistent with the existing bullets (same company, same systems/scale,`,
      `no contradiction, no duplication); each one sentence, strong past-tense verb,`,
      `concrete technology, clear judgment → outcome.`,
    ].join("\n");
  }
  ...
  for (const { storeKey } of orderedCompanies) {
    ...
    // Deterministically decide how many top-up lines this role needs (LLM produces them).
    const pname = profileNameByStoreKey.get(storeKey);
    const exp = pname ? expByName.get(pname) : undefined;
    const years = exp ? tenureYears(exp.startDate, exp.endDate) : 0;
    const need =
      preInjectionBullets.length > 0
        ? Math.min(3, Math.max(0, minLinesForTenure(years) - preInjectionBullets.length))
        : 0;
    const expectedLen = preInjectionBullets.length + need;

    let finalBullets = preInjectionBullets;
    let topUpBullets: string[] = [];
    let k2Placed: string[] = [];
    let k2Skipped: string[] = [];
    let injectionNote: string | undefined;

    const hasWork = extraction.K2.length > 0 || need > 0;
    if (runInjection && preInjectionBullets.length > 0 && hasWork) {
      const sys = injectionTemplate
        ...
        .replace(/\{topup_block\}/g, topUpBlock(storeKey, need))
        .replace(/\{topup_n\}/g, String(need));
      try {
        ...
        if (Array.isArray(parsed.bullets) && parsed.bullets.length === expectedLen) {
          finalBullets = parsed.bullets.map((b) => String(b));
          if (need > 0) topUpBullets = finalBullets.slice(preInjectionBullets.length);
          ...
          injectionNote = `Injection returned ${parsed.bullets?.length ?? 0} bullets (expected ${expectedLen}); kept pre-injection bullets.`;
