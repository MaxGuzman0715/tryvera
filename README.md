...
The production deployment at `tealbridge.online` runs the Express server on `localhost:5001` and lets [Caddy](https://caddyserver.com/) terminate TLS in front of it. Caddy auto-provisions a Let's Encrypt cert on first start and redirects HTTP→HTTPS.
...
- `.env` has `PORT=5001` (NOT `80` — Caddy needs 80 for the ACME HTTP-01 challenge and the redirect).
...
3. Start the Tryvera server on 5001:
   ```powershell
