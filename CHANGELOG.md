...
- **Profile** — add static profile `jason_angeles` (Jason Angeles, Senior/Staff
  ML Engineer, PH) with résumé experience/skills/education, plus seeded
  [`answers/jason_angeles.json`](Experiment/answers/jason_angeles.json) (address,
  phone, citizenship, EEO, relocation/hybrid, "No" to employee/relative
  questions) and [`policies/jason_angeles.json`](Experiment/policies/jason_angeles.json)
  (80%-of-range salary policy, consent default, conflict-of-interest "No"). No
  generated bullet store — kept as a static profile.
- **Enpplify v0.3.5** — logger minimized circle is light blue and shows **“L”**
  (instead of the teal “E”).
- **Enpplify v0.3.4** — logger / manual_logger get a light, semi-transparent blue
  panel theme (navy header text).
- **Enpplify v0.3.3** — manual_logger: hide the whole More/Less region (sign-up
  password, drop zone, hint, memory card all live inside it).
- **Enpplify v0.3.2** — logger / manual_logger header now reads **“Logger”** with
  no logo. Restored the sign-up password button for manual_logger (it's a fill
  feature, kept alongside Easy Fill).
- **Enpplify v0.3.1** — declutter the `manual_logger` panel: also hide Copy path,
  Download, and the drop-to-upload zone (generation/résumé affordances a manual
  logger never uses). Left with Log / Force relog, the manual company/role fields,
  Easy Fill, and the sign-up password button.

## 2026-06-24

- **Tryvify v0.3.0 — logger roles & log-only runs.**
  - New account roles **logger** and **manual_logger** (server/client types, user
    admin dropdowns).
  - **Generate** now works with neither résumé nor cover letter selected: a
    log-only run that records the JD + job link + profile. CTAs read **Log** /
    **Force relog** in that mode.
  - New **“enter company & role manually”** Tryvify setting → a panel toggle +
    company/role fields. On a log-only run with it on, the extraction LLM is
    skipped and the typed values are used.
  - **logger**: résumé/CV generation is hidden entirely (the panel reads “Log this
    application”). **manual_logger**: also no AI — Q&A and Fill all are hidden
    (Easy Fill remains) and manual company/role is forced on.
